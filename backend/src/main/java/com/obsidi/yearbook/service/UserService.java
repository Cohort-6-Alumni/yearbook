...
public class UserService {
	...
	public void completeSignup(String password) {
		...
		User user = userRepository.findByUsername(username).orElseThrow(
				() -> new UserNotFoundException(String.format("User not found for username: %s", username)));
	...
	private User updateUserProfile(Profile profile, User user) {
		...
		if (Optional.ofNullable(currentProfile).isPresent()) {
			...
			this.updateValue(profile::getMostMemorableBootcampMoment, currentProfile::setMostMemorableBootcampMoment);
